...
/**
 * This package is used by the following paper:
...
/**
 * Expand the matrix A by mirroring the boundaries.
 */
...
/**
 * Shrink the matrix A by removing the mirrored boundaries.
 */
...
/**
 * Ensure boundary conditions by mirroring.
 */
...
/**
 * Compute the gradient of the field f.
 */
...
void del2(double *f, double *del2f, int m, int n)
{
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            {
                del2f[i + j * m] = f[i + 1 + j * m] + f[i - 1 + j * m] + 
                                   f[i + (j + 1) * m] + f[i + (j - 1) * m] - 
                                   4 * f[i + j * m];
            }
        }
...
void gvf(double *f, int m, int n, double alpha, double mu, int iter, double *u, double *v)
{
    ...
    if (fmax <= fmin)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:fWrongRange", "Input f is constant.");
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 4)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:invalidNumInputs", "Four inputs required.");
    }
    if (nlhs != 2)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:invalidNumOutputs", "Two outputs required.");
    }
    
    // Get f
    double *f0 = mxGetPr(prhs[0]);
    int m = mxGetM(prhs[0]);
    int n = mxGetN(prhs[0]);
    
    if (m < 10 || n < 10)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:fWrongSize", "Input f is too small (must be at least 10x10).");
    }
    ...
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:alphaNotScalar", "Input alpha must be a scalar.");
    }
    double alpha = mxGetScalar(prhs[1]);
    if (alpha < 0)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:alphaWrongRange", "Input alpha must be non-negative.");
    }
    
    // Get mu
    if (!mxIsDouble(prhs[2]) || mxIsComplex(prhs[2]) || mxGetN(prhs[2]) * mxGetM(prhs[2]) != 1)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:muNotScalar", "Input mu must be a scalar.");
    }
    double mu = mxGetScalar(prhs[2]);
    if (mu < 0)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:muWrongRange", "Input mu must be non-negative.");
    }
    
    // Get iter
    if (!mxIsDouble(prhs[3]) || mxIsComplex(prhs[3]) || mxGetN(prhs[3]) * mxGetM(prhs[3]) != 1)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:iterNotScalar", "Input iter must be a scalar.");
    }
    int iter = (int)mxGetScalar(prhs[3]);
    if (iter < 1)
    {
        mexErrMsgIdAndTxt("MATLAB:GVF:iterWrongRange", "Input iter must be positive.");
    }
    ...
    // Create C++ pointers to copies of the output matrix
